...
Plain async functions; mcp-app's ``tools_module`` introspection picks
them up via ``gwsa.mcp.tools.__init__`` which re-exports from each
domain submodule. Each tool delegates to ``gwsa.sdk.mail`` — the SDK
is the single point of credential resolution and Google API access.
"""
...
async def list_email_labels() -> list[dict[str, Any]]:
    ...
    Uses the default Google account on the active user's profile
    (or the sole account if only one is configured). The operator
    picks a non-default account via ``gwsa-admin accounts use <name>``;
    per-tool-call account selection is planned for a later phase.
...
async def get_email_thread(thread_id: str) -> dict[str, Any]:
    ...
    """
    try:
        return mail.get_thread(thread_id=thread_id)
    except Exception as e:
